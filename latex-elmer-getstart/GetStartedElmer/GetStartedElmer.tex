...
\centering
\includegraphics[width=0.38\textwidth]{cmd-7}
\caption{Windows 11 Powershell}\label{fg:cmd-7}
\end{wrapfigure}

Windows 11 includes another method of opening a Powershell window.  Navigate to the desired directory, right click, and select the command 'Open in Terminal'.  See Figure~\ref{fg:cmd-7}\\
...
A typical use of the command prompt in Elmer will be to run Elmergrid and Elmersolver.  One can also use batch files to run Elmergrid and Elmersolver, which can be handy and sometimes faster.\\

For example, use a text editor such as Notepad to create a text file named \texttt{run.bat} with the following contents:
...
\section{Elmer and SI Units}  \label{elmer-si-units}

Elmer does not assume any particular system of units for modelling.  The user is responsible for setting units that are consistent.  The user can define a unit system based on meters, microns, or on light years, if that is more suitable for the modelling analysis.  The typical Elmer model usually can work with SI Units, with the length dimension based on meters.\\

ElmerGUI includes a material library, with basic materials such as air, water, and steel.  The material properties are defined using SI Units.  A new Elmer user that is just getting started will probably use ElmerGUI, so the user should use SI Units for the geometry and model.  The geometry generated by typical CAD programs may be defined using millimetres, and the user is responsible for converting length dimensions from millimetres to meters by using coordinate scaling.

\section{Elmer FAQ: Millimetres}  \label{elmer-faq-mm}

One frequently asked question in the Elmer forum is 'why is my simulation off by a factor of X?' or 'Elmer has a bug?'.  There are several variations of this question, but they are all caused by having geometry in millimetres and material properties in meters, leading to a solution that is off by a factor.  For example, in a 3D model if a load is applied to a face, the stress will be either N/mm2 or N/m2, for a factor of 1.0 E6.

Trying to detect and warn the user about the millimetre/meter situation is not obvious.  In an effort to help warn the user, ElmerGrid includes a feature where a bounding box for the mesh is calculated and printed to the console output.  If the largest bounding dimension exceeds a numerical value of 50, then a polite message is printed to the console, suggesting that the user consider whether millimetres need to be scaled to meters.

\section{Elmer and Coordinate Scaling}  \label{elmer-scaling}

Because converting millimetres to meters is a common task, there are multiple conversion methods that can be used.

One can start with using meters in the CAD program, instead of the default of millimetres.  If using meters in the CAD program isn't convenient, then converting later must be done.

One typical workflow consists of developing the geometry in millimetres in a CAD program, such as Salome, and exporting the geometry as a STEP file or a UNV file.  Then import the geometry into Gmsh and create the mesh in Gmsh.  Gmsh can scale the geometry from millimetres to meters before meshing, and then the exported mesh will be in meters and ready for conversion to Elmer mesh format.

Alternatively, one can create the geometry in millimetres, using a CAD program or directly in Gmsh, create the mesh in Gmsh and export the mesh from Gmsh in millimetres.  ElmerGrid can then be used to scale and convert the Gmsh mesh into an Elmer mesh.  Adding the scale command to the command line call to ElmerGrid is easy and adding 'autoclean' is also recommended.  For example, to call ElmerGrid to convert, scale with a factor of 0.001, and autoclean a Gmsh mesh file named 'mesh.msh':

\ttbegin
elmergrid 14 2 mesh -scale 0.001 0.001 0.001 -autoclean
\ttend

Finally, one can create the mesh in millimetres and use ElmerGrid to convert the mesh into Elmer mesh format, while keeping the Elmer mesh in millimetres.  One then adds the command 'Coordinate Scaling = 0.001' into the Simulation section of the sif file, and ElmerSolver will perform the scaling operation on the mesh during start up.  Note that in the sif file, one only needs to add '0.001' once, and the parser assumes that all three coordinates will be scaled by 0.001.  ElmerGUI includes the command for coordinate scaling under the Setup/Simulation menu, as shown in Figure~\ref{fg:elmergui-8}.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{elmergui-8}
\caption{Scaling and Mesh Pieces}\label{fg:elmergui-8}
\end{figure}

\section{Elmer and Conforming/Non-Conforming Meshes} \label{conforming-nonconforming}

Why talk about conforming and non-conforming meshes?  Because most new Elmer users start with problems that require conforming meshes, and unfortunately the geometry and  mesh generation process sometimes unintentionally produces non-conforming meshes, which causes the problem analysis to fail in a mysterious manner.  Trying to work with an unexpectedly non-conforming mesh has been a Frequently Asked Question in the Elmer forum.\\

Finite element meshes can be conforming or can be non-conforming meshes.  If one searches the internet for 'conforming mesh' and for 'non-conforming mesh', then the details about each type of mesh will be described.

A simple description of a conforming mesh is a mesh where every node is connected to every other node by element connections.  A non-conforming mesh is a mesh where not every node is connected to each other.  

One method of testing a mesh for conformance is to apply the Heat Equation solver to check heat conduction through the mesh.

If heat can conduct through the entire mesh, then the mesh is conforming.  An example can be found in the first ElmerGUI tutorial, 'Heat equation 3D Temperature field of a solid object'.

If heat can not conduct through the entire mesh, then the mesh is non-conforming.  The fourth ElmerGUI tutorial, 'Heat Equation 2D Axi Symmetric Steady State Radiation' includes an example of a non-conforming mesh, where there isn't a conductive connection between the two bodies, and heat is transferred solely by radiation.

Sometimes a conforming mesh is required and sometimes a non-conforming mesh is required, as demonstrated by the two referenced tutorials.

\section{ElmerGrid Conforming/Non-Conforming Meshes Check}

ElmerGrid is used to convert meshes into Elmer mesh format.  Recently ElmerGrid was extended to include a mesh conformance check as part of every mesh conversion.  The check consists of checking each node and determining what piece of the mesh each node belongs inside.  A conforming mesh will have one mesh piece, while a non-conforming mesh will have more than one mesh piece.  When converting any mesh using ElmerGrid, it's a good practice to inspect the console output for messages.\\

Running ElmerGrid on the conforming mesh in the first tutorial produces this console output:
...
Running ElmerGrid on the non-conforming mesh in the fourth tutorial produces this console output:

\ttbegin
Calculate Mesh Pieces in mesh:
Number of separate pieces in mesh is 2
The mesh is non-conforming. If not expecting a non-conforming
mesh, then refer to the Elmer User Forum for help.
\ttend

\section{ElmerSolver Conforming/Non-Conforming Meshes Test}

There are two ElmerSolver sif file commands related to conforming/non-conforming meshes.  ElmerSolver can perform a conformance check and will report the number of mesh pieces in the mesh.  Note that the command 'Calculate Mesh Pieces = True' is an information only command, and so the 'Max Output Level' must be set to 5 or greater for the information to be displayed.  ElmerSolver can be commanded to test for a desired number of mesh pieces, with 'Desired Mesh Pieces = n', where 'n' is an integer, one or greater.  If the desired number of pieces is not detected, a fatal error is triggered and the simulation halts immediately with an error message to the solver log.  Either command may be used, or both commands may be used.\\

ElmerSolver commands for conformance are added to the Simulation section of the sif file:
...
ElmerGUI includes entries for both commands under Setup/Simulation, as shown in Figure~\ref{fg:elmergui-8}.

\section{Check for coincident nodes with ElmerGrid}
...
Let's set the size of the box to be DX = 300, DY = 200, and DZ = 200, by typing the numbers into the entry boxes, followed by clicking on `Add', as shown in Figure~\ref{fg:gmsh-11}.  Elmer uses SI units, (actually Elmer is unit-less, but in practical terms think of SI units as the active units in Elmer) so when running ElmerSolver these dimensions will be in meters.  You can scale the dimensions from millimetres to meters in gmsh, in ElmerGrid, or in ElmerSolver.  Refer to section \ref{elmer-si-units} \texttt{Elmer and SI Units}, section  \ref{elmer-faq-mm} \texttt{Elmer FAQ: Millimetres}, and section \ref{elmer-scaling} \texttt{Elmer and Coordinate Scaling}  for more details.
...
We now have two boxes that are overlapping, and we need to turn them into three separate volumes.\\

As a side note, if we meshed the two overlapping boxes right now, we would create a non-conforming mesh.  We need to convert the two boxes into three adjacent volumes that share nodes on the boundaries.  Refer to section \ref{conforming-nonconforming} \texttt{Elmer and conforming and non-conforming meshes} for more details about working with conforming meshes.\\

In order to work on volumes, we need to be able to select the volumes, but we only see points and line segments in the viewport.  With the mouse over the viewport, double left click (LMB) and the viewport menu will pop up.  Select `Geometry visibility' and click on `Volumes', as shown in Figure~\ref{fg:gmsh-18}.  You can see two yellow spheres, one for each of the two boxes, in the viewport, ready to be selected.
...
Let's take a look at the geo file, and we see two boxes and a BooleanFragments command.

\newpage

\noindent File: getstart1.geo
\verbatiminput{GetStartedElmer/getstart1.geo}

Click on `Tools -> Statistics' in the top bar, to open a pop up window with statistics about our model, as shown in Figure~\ref{fg:gmsh-23a}.  Note there are three tabs in the Statistics window, take a look at the 'Geometry' tab.  Notice that there are 16 surfaces and 3 volumes in the model, which is our desired outcome from the geometry creation process.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{gmsh-23a}
\caption{Statistics for the Geometry tab}\label{fg:gmsh-23a}
\end{figure}

\subsection{Coherence}

Any discussion of the 'Boolean Fragments' command won't be complete without discussing the 'Coherence' command.\\

\noindent Here is a quote from the Gmsh 14.3.1 manual:\\

\texttt{Coherence;}

\texttt{Remove all duplicate elementary entities (e.g., points having identical
...
page 252). With the OpenCASCADE geometry kernel, Coherence is simply
a shortcut for a BooleanFragments operation on all entities, with the Delete
operator applied to all operands.}\\

The Coherence command will affect all entities, and so there is no need to specify or select the bodies, making the command very simple to use.  The net effect of Coherence will be to make overlapping bodies that will produce a non-conforming mesh into adjacent bodies that will produce a conforming mesh.  Search the Elmer Forum for more examples of using Coherence to ensure a conforming mesh.\\

Here's our example using the Coherence command:\\

\noindent File: getstart2.geo
\verbatiminput{GetStartedElmer/getstart2.geo}
...
\subsection{Save mesh}

We are ready to save the mesh with the default gmsh settings.  Click on `File -> Save Mesh', as shown in Figure~\ref{fg:gmsh-39}.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{gmsh-39}
\caption{File -> Save Mesh}\label{fg:gmsh-39}
\end{figure}

After clicking on 'Save Mesh', the mesh is saved to a *.msh file, using the model name as root and adding the msh extension.  The bottom message says 'Done writing 'C:/Code/Elmer/getstart1.msh'.  By default, the mesh will be in the most recent gmsh format, (currently format 4.1), and in ASCII format.

If needed, the mesh can be saved in alternative gmsh file formats, such as in binary format, by using the Export command.

\subsection{Export mesh}

If other than the default file format options are needed, then instead of saving the mesh, gmsh has a second menu to export the mesh with several options.  Click on `File -> Export', as shown in Figure~\ref{fg:gmsh-29}.  We will use the gmsh file type of 'msh', and 'Save as type: Guess From Extension'.
...
A pop up window will ask you to choose a gmsh Format.  For example, select `Version 2 ASCII', as shown in Figure~\ref{fg:gmsh-31}.  Also be sure to check the box to `Save all elements'.  Later on, if desired, come back here and try the other format options.  For this example, Version 1 ASCII, Version 2 ASCII, Version 2 Binary, Version 4 ASCII, and Version 4 Binary, all load properly in ElmerGUI and in ElmerGrid.
...
\noindent The most relevant test cases in the distribution include:\\
    ...
    reynolds2: film pressure between rotating non-concentric cylinders in the plane with periodic BCs
